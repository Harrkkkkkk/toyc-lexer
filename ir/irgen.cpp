...
void IRGenerator::generate(std::shared_ptr<CompUnit> ast) {
    if (ast) {
        ...
        if (config.enableOptimizations) {
            // Debug IR printing (commented out)
            // std::cerr << "=== IR Before Optimization ===" << std::endl;
            // for (auto& instr : instructions) {
            //     std::cerr << instr->toString() << std::endl;
            // }
            // std::cerr << "============================" << std::endl;
            optimize();
        }
...
void IRGenerator::optimize() {
    ...
    algebraicSimplification();    // 应用代数恒等式简化
    
    // 第二轮：常量和复制传播
    constantPropagationCFG();     // 在代码中传播常量值
    copyPropagationCFG();         // 复制传播优化
    constantFolding();            // 再次常量折叠
    algebraicSimplification();    // 再次代数简化
    
    // 第三轮：强度削减
    strengthReduction();          // 强度削减（乘法转移位等）
    
    // 第四轮：清理优化
    constantPropagationCFG();     // 再次常量传播
    constantFolding();            // 再次常量折叠
    algebraicSimplification();    // 再次代数简化
    strengthReduction();          // 再次强度削减
...
void IRGenerator::algebraicSimplification() {
    for (size_t i = 0; i < instructions.size(); ++i) {
        ...
        bool leftIsConst = (binOp->left->type == OperandType::CONSTANT);
        bool rightIsConst = (binOp->right->type == OperandType::CONSTANT);
        
        // 跳过两边都是常量的情况（由常量折叠处理）
        if (leftIsConst && rightIsConst) continue;
        
        std::shared_ptr<Operand> simplifiedResult = nullptr;
        
        // 应用代数恒等式
        switch (binOp->opcode) {
            case OpCode::ADD:
                // x + 0 = x
                if (rightIsConst && binOp->right->value == 0) {
                    simplifiedResult = binOp->left;
                }
                // 0 + x = x  
                else if (leftIsConst && binOp->left->value == 0) {
                    simplifiedResult = binOp->right;
                }
                break;
            ...
            case OpCode::SUB:
                ...
                // x - x = 0 (仅当操作数相同时)
                else if (binOp->left->name == binOp->right->name &&
                         binOp->left->type == binOp->right->type &&
                         (binOp->left->type == OperandType::VARIABLE || 
                          binOp->left->type == OperandType::TEMP)) {
                    simplifiedResult = std::make_shared<Operand>(0);
                }
                break;
                
            case OpCode::MUL:
                // x * 1 = x
                if (rightIsConst && binOp->right->value == 1) {
                    simplifiedResult = binOp->left;
                }
                // 1 * x = x
                else if (leftIsConst && binOp->left->value == 1) {
                    simplifiedResult = binOp->right;
                }
                // x * 0 = 0
                else if (rightIsConst && binOp->right->value == 0) {
                    simplifiedResult = std::make_shared<Operand>(0);
                }
                // 0 * x = 0
                else if (leftIsConst && binOp->left->value == 0) {
            ...
            case OpCode::DIV:
                ...
                // 注意：不能简化 0 / x = 0，因为当 x = 0 时会产生除零错误
                // 移除这个优化以保持程序语义
                break;
        ...
        // 如果找到简化，用赋值指令替换
        if (simplifiedResult) {
            auto assignInstr = std::make_shared<AssignInstr>(binOp->result, simplifiedResult);
            instructions[i] = assignInstr;
        }
    }
}
    ...
    const std::unordered_map<BlockID, IRGenerator::BasicBlock>& blocks)
{
    ...
    for (auto blkId : loopBlocks) {
        ...
        for (auto& inst : it->second.instructions) {
            auto definedVars = IRAnalyzer::getDefinedVariables(inst);
    ...
    BlockID block) 
{
    ...
    if (it != loopDefs.end()) {
        ...
        for (auto& var : it->second) {
            // 强制将变量状态设为Top（覆盖可能的常量传播结果）
            inMap[var] = {LatticeKind::Top, 0}; 
        }
    }
}
...
std::vector<std::shared_ptr<IRGenerator::BasicBlock>> IRGenerator::buildBasicBlocks() {
    ...
    std::unordered_map<std::string, std::string> oldToNew; // 记录第一次出现的原标签到新标签的映射
    std::unordered_map<std::string, int> labelCounter;     // 记录每个标签出现的次数
    for (auto &instr : this->instructions) {
        if (auto lbl = std::dynamic_pointer_cast<LabelInstr>(instr)) {
            std::string origLabel = lbl->label;
            if (seenLabels.count(lbl->label)) {
                // 重复标签，生成唯一新名字
                int count = ++labelCounter[origLabel];
                std::string newName = origLabel + "_dup" + std::to_string(count);
                lbl->label = newName;
                // 不更新oldToNew，保持第一次出现的映射
            } else {
                // 第一次出现，记录原始标签
                oldToNew[origLabel] = origLabel;
                labelCounter[origLabel] = 0;
            }
...
void IRGenerator::constantPropagationCFG() {
    // std::cerr << "Entering constantPropagationCFG" << std::endl;
    ...
    for (auto& edge : backEdges) {
        ...
        // std::cerr << "Loop Header " << toBlk << " Defs: ";
//         for (auto& d : defSet) std::cerr << d << " ";
        // std::cerr << std::endl;
    ...
    // 将所有使用但未定义的变量（参数/全局）在入口块设为 Top
    for (const auto& var : usedVars) {
        if (definedVars.find(var) == definedVars.end()) {
            // 这是一个输入变量 (参数或全局)
            inMap[0][var] = LatticeValue{LatticeKind::Top, 0};
            // std::cerr << "Identified input variable: " << var << " -> TOP" << std::endl;
        }
    }
    ...
    while (!q.empty()) {
        ...
        if (blk->predecessors.empty()) {
            ...
            // 清除循环定义变量的常量状态（每次都清理，不是只清理一次）
            // 因为worklist算法中同一块可能被多次访问
        ...
        // std::cerr << "Block " << bid << " (" << blk->label << ") IN: ";
        // for (auto& [k, v] : inMap[bid]) {
        //     if (v.kind == LatticeKind::Constant) std::cerr << k << "=" << v.constantValue << " ";
        //     else if (v.kind == LatticeKind::Unknown) std::cerr << k << "=UNK ";
        //     else if (v.kind == LatticeKind::Top) std::cerr << k << "=TOP ";
        // }
        // std::cerr << std::endl;
...
void IRGenerator::deadCodeElimination() {
    ...
    // DEBUG: Print live sets (commented out)
    // std::cerr << "=== Liveness Analysis Result ===" << std::endl;
    // for (auto& block : basicBlocks) {
    //     std::cerr << "Block " << block->id << " (" << block->label << "):" << std::endl;
    //     std::cerr << "  LiveIn: ";
    //     for (auto& v : live_in[block]) std::cerr << v << " ";
    //     std::cerr << std::endl;
    //     std::cerr << "  LiveOut: ";
    //     for (auto& v : live_out[block]) std::cerr << v << " ";
    //     std::cerr << std::endl;
    // }
    // std::cerr << "================================" << std::endl;
...
using CopyMap = std::map<std::string, std::shared_ptr<Operand>>;
...
bool copyMapEqual(const CopyMap& a, const CopyMap& b) {
    ...
    for (auto itA = a.begin(), itB = b.begin(); itA != a.end(); ++itA, ++itB) {
        ...
        if (itA->second->name != itB->second->name) return false;
    }
    return true;
}
...
CopyMap meetCopyMaps(const CopyMap& a, const CopyMap& b) {
    ...
    for (auto& [var, mappedVar] : a) {
        auto it = b.find(var);
        if (it != b.end() && it->second->name == mappedVar->name) {
...
void replaceCopyUses(std::shared_ptr<IRInstr>& instr, const CopyMap& env) {
    ...
    for (auto& useVar : uses) {
        ...
        std::shared_ptr<Operand> finalOp = nullptr;
        ...
        // 如果找到了替换（finalOp不为空），则执行替换
        if (finalOp) {
            IRAnalyzer::replaceUsedVariable(instr, useVar, finalOp);
        }
    }
}
...
void IRAnalyzer::replaceUsedVariable(std::shared_ptr<IRInstr>& instr, 
    ...
    const std::shared_ptr<Operand>& newOp) 
{
...
void IRGenerator::commonSubexpressionElimination() {
    ...
    for (auto& blk : blocks) {
        ...
        for (size_t i = 0; i < blk->instructions.size(); ++i) {
            ...
            if (!defVar.empty()) {
                // 只有当表达式的操作数不包含被定义的变量时，才将其加入可用表达式集合
                // 否则（如 x = x + 1），表达式的值已经改变，不能复用
                if (e.lhs != defVar && e.rhs != defVar) {
...
/**
 * 强度削减优化（Strength Reduction）
 * 将开销大的运算替换为开销小的等价运算：
 * - x * 2^n => x << n (左移)
 * - x / 2^n => x >> n (右移，对于正数)
 * - x % 2^n => x & (2^n - 1) (按位与，对于正数)
 */
void IRGenerator::strengthReduction() {
    // 判断一个数是否是2的幂，并返回幂次
    auto isPowerOfTwo = [](int val, int& power) -> bool {
    ...
    for (size_t i = 0; i < instructions.size(); ++i) {
        ...
        int power = 0;
        std::shared_ptr<BinaryOpInstr> newOp = nullptr;
        
        // x * 2^n => x << n
        if (binOp->opcode == OpCode::MUL && rightIsConst && 
            isPowerOfTwo(binOp->right->value, power)) {
            newOp = std::make_shared<BinaryOpInstr>(
                OpCode::SHL, binOp->result, binOp->left, 
                std::make_shared<Operand>(power));
        }
        // 2^n * x => x << n
        else if (binOp->opcode == OpCode::MUL && leftIsConst && 
                 isPowerOfTwo(binOp->left->value, power)) {
            newOp = std::make_shared<BinaryOpInstr>(
                OpCode::SHL, binOp->result, binOp->right, 
                std::make_shared<Operand>(power));
        }
        // 注释掉除法优化 - 可能导致负数问题
        // x / 2^n => x >> n (算术右移)
        // else if (binOp->opcode == OpCode::DIV && rightIsConst && 
        //          isPowerOfTwo(binOp->right->value, power)) {
        //     newOp = std::make_shared<BinaryOpInstr>(
        //         OpCode::SHR, binOp->result, binOp->left, 
        //         std::make_shared<Operand>(power));
        // }
...
void IRGenerator::loopInvariantCodeMotion() {
    ...
    for (auto& edge : backEdges) {
        ...
        // 查找循环中的常量赋值（最简单的LICM）
        std::vector<std::pair<int, size_t>> toMove; // (blockId, instrIndex)
        
        for (int bid : loopBlocks) {
            auto& instrs = blocks[bid]->instructions;
            for (size_t i = 0; i < instrs.size(); ++i) {
                ...
                // 跳过有副作用的指令
                if (isSideEffectInstr(instr)) continue;
                
                // 检查是否是常量运算（两个操作数都是常量）
                bool leftConst = (binOp->left->type == OperandType::CONSTANT);
                bool rightConst = (binOp->right->type == OperandType::CONSTANT);
                
                if (leftConst && rightConst) {
                    // 这是循环不变量，可以外提
                    toMove.push_back({bid, i});
                }
            }
        }
        
        // 执行外提（从后向前移除指令避免索引失效）
        std::vector<std::shared_ptr<IRInstr>> movedInstrs;
        std::sort(toMove.rbegin(), toMove.rend());
        
        for (auto& [bid, idx] : toMove) {
            auto instr = blocks[bid]->instructions[idx];
            movedInstrs.insert(movedInstrs.begin(), instr);
            blocks[bid]->instructions.erase(blocks[bid]->instructions.begin() + idx);
        }
        
        // 将外提的指令插入到循环头之前
        if (!movedInstrs.empty() && toBlk > 0) {
            auto& headerInstrs = blocks[toBlk]->instructions;
            auto insertPos = headerInstrs.begin();
            
            // 跳过标签
            while (insertPos != headerInstrs.end() && 
                   std::dynamic_pointer_cast<LabelInstr>(*insertPos)) {
                ++insertPos;
            }
            
            headerInstrs.insert(insertPos, movedInstrs.begin(), movedInstrs.end());
        }
    }
